...
export function NewCountryForm() {
  ...
  return (
    <section>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 mb-2 lg:mb-0">
          {["name", "emoji", "code"].map((field) => (
            <div key={field} className="flex flex-col gap-1">
              ...
              <input
                ...
                className="bg-white rounded-xs p-1 border border-gray-400 w-full max-w-[100%] sm:max-w-[70%] hover:border-gray-500"
                required
              />
            </div>
          ))}
        ...
        <button
          type="submit"
          disabled={loading}
          className="bg-[#F7146B] text-white p-3 sm:p-4 rounded-md hover:bg-[#F7146B]/90 hover:cursor-pointer my-auto disabled:opacity-50 w-full h-fit sm:h-15 sm:w-18 ml-auto"
        >
          {loading ? <Loader /> : "Add"}
        </button>
      </form>
